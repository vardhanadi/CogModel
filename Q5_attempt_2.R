AllComplexStratergies <- function(numberform) # so, This is the function that generates combinations along with interleaving 

    # we are testing it here with a fixed fone number
{
    phoneStringLength <- 11
    # numberform <- "11111111111"
    numberform <- numeric(length = 11)
    ...
    ComplexStrategiestable_InFunction <- strategies
}
...
RunAllComplexStratergies <- function(nrSimulation,phoneNumber)
{
    ...
    source(AllComplexStratergies) # This was just an attempt in making this work 
    velocity <- c()
