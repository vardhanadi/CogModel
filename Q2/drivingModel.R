...
steeringTimeOptions <- c(1, 2, 4, 6, 8, 12)
...
#simpleSetOfStrategyVariations <- FALSE
...
#### Main function to run the code. For example: runAllSimpleStrategies(5,"07854325698") will run 5 simulations for each (simple) strategy on the phone number to the right.
#The default assumption is that the chunk boundary is between the 5th and 6th digit
...
runAllSimpleStrategies <- function(nrSimulations,phoneNumber)
{
	...
	c <- with(agrResultsMeanDrift,plot(TrialTime/1000,abs(dev),pch=21,bg="black",col= rainbow(10),log="x",type = "l",xlab="Dial time (s)",ylab="Average Lateral Deviation (m)"))
...
runAllSimpleStrategies(3, "07854325698")

##RunAllComplex Stratergies 

AllComplexStratergies <- function()
{
...
RunAllComplexStratergies <- function(nrSimulation, phoneNumber)
{
    ...
    #source(AllComplexStratergies,local = TRUE)
    velocity <- c()
...
RunAllComplexStratergies(3, "11111111111")
